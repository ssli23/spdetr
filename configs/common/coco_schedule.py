...
def default_X_scheduler(num_X):
    ...
    """
    # total number of iterations assuming 16 batch size, using 1440000(120000*12)/16=90000
    # my own dataset total number of iterations assuming 8 batch size, using 16032(1336*12)/8=2004
    # total_steps_16bs = num_X * 90000
    total_steps_16bs = num_X * 2004
    ...
    if num_X <= 2:
        scheduler = L(MultiStepParamScheduler)(
            ...
            milestones=[1336, 1781, 2004],
        )
    else:
        scheduler = L(MultiStepParamScheduler)(
            values=[1.0, 0.1, 0.01],
            # milestones=[total_steps_16bs - 60000, total_steps_16bs - 20000, total_steps_16bs],
            milestones=[total_steps_16bs - 1336, total_steps_16bs - 445, total_steps_16bs],
        )
...
def default_coco_scheduler(epochs=50, decay_epochs=40, warmup_epochs=0):
    ...
    Returns:
        DictConfig: configs that define the multiplier for LR during training
    """
    # total number of iterations assuming 16 batch size, using 120000/16=7500
    # HRT2: my own dataset total number of iterations assuming 8 batch size, using 1336/8=167
    # LRT2: my own dataset total number of iterations assuming 6 batch size, using 821/6=137
    # total_steps_16bs = epochs * 7500
    total_steps_16bs = epochs * 137
    # decay_steps = decay_epochs * 7500
    decay_steps = decay_epochs * 137
    # warmup_steps = warmup_epochs * 7500
    warmup_steps = warmup_epochs * 137
    scheduler = L(MultiStepParamScheduler)(
        values=[1.0, 0.1],
...
lr_multiplier_50ep = default_coco_scheduler(50, 40, 0)  # use this
...
lr_multiplier_24ep_warmup = default_coco_scheduler(24, 20, 1e-3)
